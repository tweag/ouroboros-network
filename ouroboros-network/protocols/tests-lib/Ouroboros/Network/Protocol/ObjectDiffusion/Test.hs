...
-- With 'createPipelineTestChannels', the peer is responsible for not exceeding the channel buffer size, and the test will fail if it does.
-- So we make sure the buffer size is greater or equal than the upper bound of pipelined requests, which is 2 * 'testMaxUnacked'
-- (see how this upper bound is computed in 'NodeToNode.hs' -> 'MiniProtocolParameters' -> 'perasCertDiffusionMaxObjectsUnacknowledged')

propDemoChannelST
  :: ObjectDiffusionTestParams
  -> ChannelSize
  -> Property
propDemoChannelST params@ObjectDiffusionTestParams{testObjects, testMaxUnacked} chanSize = runSimOrThrow $ do
    let actualChanSize = positiveWord16ToNat chanSize `max` 2 * (positiveWord16ToNat testMaxUnacked)
    (clientChan, serverChan) <- createPipelineTestChannels actualChanSize
...
propDemoChannelIO
  ...
  -> ChannelSize
  -> Property
propDemoChannelIO params@ObjectDiffusionTestParams{testObjects, testMaxUnacked} chanSize = ioProperty $ do
    let actualChanSize = positiveWord16ToNat chanSize `max` 2 * (positiveWord16ToNat testMaxUnacked)
    (clientChan, serverChan) <- createPipelineTestChannels actualChanSize
    objects <- objectDiffusionDemoPipelined clientChan serverChan params
    pure $ objects === fromDistinctList testObjects
...
propDemoChannelBufferedST params@ObjectDiffusionTestParams{testObjects} chanSize = runSimOrThrow $ do
    (clientChan, serverChan) <- createConnectedBufferedChannels (positiveWord16ToNat chanSize)
    objects <- objectDiffusionDemoPipelined clientChan serverChan params
    pure $ objects === fromDistinctList testObjects
